...
int Labeler::createAlphabet(const vector<Instance>& vecInsts) {
	...
	for (numInstance = 0; numInstance < vecInsts.size(); numInstance++) {
		...
		const vector<vector<string> > &sparsefeatures =
				pInstance->sparsefeatures;
...
void Labeler::train(const string& trainFile, const string& devFile,
	...
	m_classifier.init(wordEmb, m_options.wordcontext, charEmb, m_options.charcontext, m_labelAlphabet.size(), m_options.charhiddenSize, m_options.atomLayers, m_featAlphabet.size());
